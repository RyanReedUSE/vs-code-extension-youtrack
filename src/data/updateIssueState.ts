 ...
 */
export const updateIssueState = async (issueId: string) => {
  ...
  const updatedIssue = await axios
    ...
    .then((response) => {
      vscode.window.showInformationMessage(`${issueId} state updated to ${updatedStatus.name}`);
    ...
    .catch((err) => {
      vscode.window.showErrorMessage(`Issue updating state for ${issueId}`);
      return;
    });
