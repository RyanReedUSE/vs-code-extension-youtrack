...
export default class Config extends React.Component<IConfigProps, IConfigState> {
        ...
        <ul className="">
          {users && users.length > 0
            ? users.map((user, userIndex) => {
                const roles = user.roles && user.roles.length > 0 ? user.roles.join(',') : null;
                ...
                return (
                  <li key={userIndex}>
                    ...
                    <br />
                    Roles : {roles}
                    <input type="text" placeholder="Add Role" onKeyUp={(event) => this.onAddRole(event, userIndex)} />
                  </li>
                );
              })
            : null}
        </ul>
        <input type="text" placeholder="Add User" onKeyUp={(event) => this.onAddUser(event)} />
      </React.Fragment>
    );
  }
  ...
  render() {
    return (
      <React.Fragment>
        <h1>Config name : {this.state.config.name}</h1> {this.state.config.description}
        {this.renderUsers(this.state.config.users)}
        <br />
        <input className="save" type="button" value="Save the configuration" onClick={() => this.saveConfig()} />
      </React.Fragment>
    );
  }
